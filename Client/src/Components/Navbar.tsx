...
const Navbar: React.FC = () => {
  ...
  // Get user/org data from store
  const { user, org, logout } = useUserStore((state) => state);
  const currentRole = user?.role || org?.role; // get role if exists

  const handleLogout = async () => {
    setLoading(false);
    try {
      const endpoint = org
        ? `${backendurl}/auth/logoutOrg`
        : `${backendurl}/auth/logoutUser`;

      const response = await axios.post(endpoint, {}, { withCredentials: true });
      logout();
      toast.success(response.data.data || "User logout successful!");
      navigate("/login");
    } catch (error: any) {
      toast.error(
  ...
  const navigationItems = [
    { name: "User Dashboard", path: "/user-dashboard", hideForAdmin: true },
    { name: "Admin Dashboard", path: "/admin-dashboard", onlyForAdmin: true },
          ...
          <div className="flex items-center">
            ...
            {(user || org) && (
              <div className="hidden md:flex ml-10 space-x-8">
                {navigationItems
                  .filter((item) => {
                    // Hide User Dashboard for admin
                    if (item.hideForAdmin && currentRole === "admin") return false;
                    // Show Admin Dashboard only for admin
                    if (item.onlyForAdmin && currentRole !== "admin") return false;
                    return true;
                  })
                  .map((item) => (
                    <Link
