...
const { setUser, setOrg } = useUserStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.statusCode ===200  && response.data.data.role ==="ADMIN") {


        toast.success(response.data.message || "Organization login successful!");
        setOrg(response.data.data);
        navigate("/admin-dashboard"); // Navigate to the dashboard or target page
      }
        if (response.data.statusCode === 200  && response.data.data.role ==="USER") {

        toast.success(response.data.message || "User login successful!");
        setUser(response.data.data);
        navigate("/user-dashboard"); // Navigate to the dashboard or target page
      }
    } catch (err: any) {
      const errorMessage =
