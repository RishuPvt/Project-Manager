...
import { Calendar, User, AlertCircle, Edit, Trash2, MoreVertical } from 'lucide-react';
...
const KanbanCard: React.FC<KanbanCardProps> = ({ task, onEdit, onDelete }) => {
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      ...
      case 'medium': return 'bg-orange-100 text-orange-800 dark:bg-orange-900 dark:text-orange-200 border-orange-200 dark:border-orange-800';
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <span className="text-lg flex-shrink-0">{getPriorityIcon(task.priority)}</span>
          </div>
          {isOverdue && (
